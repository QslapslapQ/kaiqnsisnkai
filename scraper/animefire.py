...
def search(query: str):
    ...
    for card in soup.select("article.cardUltimosEps, div.divCardUltimosEps"):
        ...
        # imagem com fallbacks
        image_url = None
        if img:
            image_url = img.get("data-src") or img.get("src") or img.get("data-lazy-src")

        # se ja existe esse base_slug, so adiciona a versao
...
def get_anime(slug: str):
    ...
    # seletores amplos - pega o primeiro que existir
    def sel(*args):
        for s in args:
    ...
    # imagem principal da pagina do anime (coluna esquerda, nao cards laterais)
    image_url = None
    for img in soup.select("img"):
        src = img.get("data-src") or img.get("src") or ""
        classes = img.get("class", [])
        # pula imagens de cards de episodios e avatares
    ...
    # generos: links dentro de animeInfo, limpa classificacao indicativa
    genre_tags = soup.select("div.animeInfo a")
    genres_text = ", ".join(
        a.text.strip() for a in genre_tags
        if a.text.strip() and not __import__("re").match(r"^A[0-9]+$", a.text.strip())
    ) or None

    # score: span que contem numero (ex: 8.5)
    score_text = None
    ...
    # episodios — pega numero do FINAL da URL
    episodes = []
    seen_eps = set()
    for ep_link in soup.select("a.lEp, a[href*='/animes/']"):
        ...
        # URL padrao: /animes/naruto/1 ou /animes/naruto-todos-os-episodios/1
        ...
        episodes.append({
            "episode": ep_num,
            "link": ep_href if ep_href.startswith("http") else BASE + ep_href
        })
...
# rota de debug — lista todos os seletores encontrados numa pagina
